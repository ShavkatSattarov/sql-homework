...
--6
SELECT Orders.OrderID, Customers.CustomerName, Employees.FirstName
FROM Orders
...
--9
SELECT Employees.FirstName, Products.ProductName
FROM Employees
CROSS JOIN Products;
...
--18
SELECT Orders.OrderID, Customers.Country, Employees.Country
FROM Orders
JOIN Customers ON Orders.CustomerID = Customers.CustomerID
JOIN Employees ON Orders.EmployeeID = Employees.EmployeeID;
...
FROM Products
JOIN Suppliers ON Products.SupplierID = Suppliers.SupplierID
WHERE Products.Country = Suppliers.Country;


--20
SELECT Employees.FirstName, Customers.CustomerName
FROM Employees
JOIN Customers ON Employees.Country = Customers.Country;
...
--23
SELECT Employees.FirstName, COUNT(Orders.OrderID) AS OrderCount
FROM Employees
JOIN Orders ON Employees.EmployeeID = Orders.EmployeeID
...
--25
SELECT Suppliers.CompanyName, SUM(OrderDetails.UnitPrice * OrderDetails.Quantity) AS TotalAmount
FROM Suppliers
