...
--Solution
select DepartmentName, COUNT(EmployeeID) as EmpCount
from Employees
...
--Solution
select CustomerID, sum(SaleAmount) as TotSales
from Sales
group by CustomerID
order by CustomerID;
...
--Solution
select DepartmentName, COUNT(EmployeeID) as NumbEmployees
from Employees
group by DepartmentName
...
--Solution
SELECT P.Category, SUM(S.SaleAmount) AS TotSales, AVG(S.SaleAmount) AS AvgSales
FROM Sales S
...
--Solution
select count(EmployeeID) as NumofEmp
from Employees
...
--Solution
select DepartmentName, AVG(Salary) as AvgSalary, COUNT(EmployeeID) as NumofEmp
from Employees
group by DepartmentName
order by DepartmentName;
...
--Solution
select CustomerID, sum(case when Freight > 50 then Freight else 0 end) as TotalFreightOver50,
