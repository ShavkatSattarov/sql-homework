...
--Solution
-- 1. Employees with salary > 50000 and their departments
...
-- 3. All employees and their department names, including those without a department
SELECT E.Name AS EmployeeName, D.DepartmentName
FROM Employees E
LEFT JOIN Departments D ON E.DepartmentID = D.DepartmentID;

-- 4. Suppliers and the products they supply, including suppliers with no products
SELECT S.SupplierName, P.ProductName
FROM Suppliers S
LEFT JOIN Products P ON S.SupplierID = P.SupplierID;

-- 5. Orders and their payments, include unmatched from both sides
...
WHERE Orders.OrderDate >= '2023-01-01' AND Customers.CustomerName = 'John';



-- 12. Customers who placed at least one order
SELECT C.FirstName, C.LastName, COUNT(O.OrderID) AS TotalOrders
FROM Customers C
JOIN Orders O ON C.CustomerID = O.CustomerID
GROUP BY C.FirstName, C.LastName;

-- 13. Only matched employees and departments
SELECT E.Name AS EmployeeName, D.DepartmentName
FROM Employees E
INNER JOIN Departments D ON E.DepartmentID = D.DepartmentID;

-- 14. Employees reporting to the same manager
...
-- 19. Employees earning more than avg in their department
...
) AS AvgDept ON E.DepartmentID = AvgDept.DepartmentID
...
-- 24. Sales with amount > 500 including product names
...
WHERE Courses.CourseName = 'Math' OR 'Science';
